...
const Fornecedores: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Tem certeza que deseja excluir este fornecedor?')) {
      try {
        ...
        setError('Erro ao excluir fornecedor. Verifique se não há lançamentos associados.');
  ...
  const filteredFornecedores = fornecedores.filter(fornecedor => 
    ...
    fornecedor.identificador.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <Layout>
      <div className="py-6">
        ...
        <div className="mb-4">
          <input
            type="text"
            placeholder="Buscar por nome, CPF/CNPJ ou identificador..."
        ...
        {loading ? (
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md p-6 text-center">
            <p className="text-gray-500">
              {searchTerm ? 'Nenhum fornecedor encontrado com os critérios de busca.' : 'Nenhum fornecedor cadastrado.'}
            </p>
          </div>
        ) : (
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      <div className="flex flex-col">
                        <p className="text-sm font-medium text-blue-600 truncate">{fornecedor.nome}</p>
                        <p className="text-sm text-gray-500">
                          CPF/CNPJ: {fornecedor.cpf_cnpj} | ID: {fornecedor.identificador}
                        </p>
                      </div>
                    ...
                    <div className="mt-2 sm:flex sm:justify-between">
                      ...
                      <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                        ...
                        {fornecedor.email || 'Email não cadastrado'}
                      </div>
                    </div>
                  </div>
                </li>
